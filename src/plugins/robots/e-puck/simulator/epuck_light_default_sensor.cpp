...
#include "epuck_light_default_sensor.h"
...
namespace argos {
   ...
   CEPuckLightDefaultSensor::CEPuckLightDefaultSensor() :
   ...
   void CEPuckLightDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the e-puck light default sensor", ex);
      }
   }
   ...
   void CEPuckLightDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in e-puck default light sensor", ex);
      }
   }
   ...
   void CEPuckLightDefaultSensor::Update() {
      /* Erase readings */
      ...
      for(CSpace::TMapPerType::iterator it = mapLights.begin();
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               cAngleLightWrtEPuck = cRobotToLight.GetZAngle();
               cAngleLightWrtEPuck -= cOrientationZ;
               ...
               for(SInt32 nIndexOffset = 0; nIndexOffset < 8; ++nIndexOffset) {
                  cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtEPuck - m_tReadings[nIndexOffset].Angle).SignedNormalize());
   ...
   void CEPuckLightDefaultSensor::Reset() {
   ...
   REGISTER_SENSOR(CEPuckLightDefaultSensor,
                   "epuck_light", "default",
                   "Lorenzo Garattoni [lgaratto@ulb.ac.be]",
                   "1.0",
                   "The e-puck light sensor",
