#include "xpuck_camera_client_main_window.h"
#include "xpuck_camera_client_image_grabber_configurator.h"
#include "xpuck_camera_client_blob_detector_configurator.h"
#include "xpuck_camera_client_distance_converter_configurator.h"
...
CXPuckCameraClientMainWindow::CXPuckCameraClientMainWindow(const QString& str_hostname,
                                                           quint16 un_port) {
...
void CXPuckCameraClientMainWindow::closeEvent(QCloseEvent* pc_event) {
}

/****************************************/
/****************************************/

void CXPuckCameraClientMainWindow::CreateActions() {
    /* Save configuration */
...
void CXPuckCameraClientMainWindow::CreateToolBar() {
...
void CXPuckCameraClientMainWindow::CreateImageWidgets() {
    ...
    m_pcImageGrabberConf = new CXPuckCameraClientImageGrabberConfigurator(pcCentralWidget);
    ...
    m_pcBlobDetectorConf = new CXPuckCameraClientBlobDetectorConfigurator(pcCentralWidget);
    UInt32 unNumDetectors = cBuffer.PopFront<UInt32>();
    for(size_t i = 0; i < unNumDetectors; ++i) {
        CXPuckCameraClientBlobDetectorConfiguratorTab* pcDetector = new CXPuckCameraClientBlobDetectorConfiguratorTab();
	...
	m_pcDistanceConverterConf = new CXPuckCameraClientDistanceConverterConfigurator(pcCentralWidget);
...
void CXPuckCameraClientMainWindow::SendCommand(argos::ECameraConfiguratorCommand e_command) {
...
void CXPuckCameraClientMainWindow::SetStatusMsg(const QString& str_message) {
...
void CXPuckCameraClientMainWindow::GetAttributes() {
    /* Send request to get attributes */
    SendCommand(argos::COMMAND_GET_ATTRIBUTES);
    /*
       * Receive data and configure widgets
    ...
    UInt32 unNumDetectors = cBuffer.PopFront<UInt32>();
    for(size_t i = 0; i < unNumDetectors; ++i) {
        CXPuckCameraClientBlobDetectorConfiguratorTab* pcDetector = m_pcBlobDetectorConf->GetDetector(i);
        pcDetector->SetImageWidth(m_pcImageGrabberConf->GetImageWidth());
        pcDetector->SetImageHeight(m_pcImageGrabberConf->GetImageHeight());
...
void CXPuckCameraClientMainWindow::SetAttributes() {
...
void CXPuckCameraClientMainWindow::PauseChecked() {
...
void CXPuckCameraClientMainWindow::RefreshTimerTimeout() {
    if(!m_pcActionPause->isChecked()){
...
void CXPuckCameraClientMainWindow::Refresh() {
...
void CXPuckCameraClientMainWindow::SaveConfiguration() {
    m_pcRefreshTimer->stop();
    SetAttributes();
...
int main(int n_argc, char** pch_argv) {
    ...
    argos::CXPuckCameraClientMainWindow cMainWindow(pch_argv[1],
