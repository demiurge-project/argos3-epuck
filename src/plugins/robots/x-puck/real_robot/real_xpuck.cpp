/**
 * @file <argos3/plugins/robots/x-puck/real_robot/real_xpuck.cpp>
 *
 * This class provides the real x-puck robot implementation. In
...
bool CRealXPuck::bGoHome() const
{
    return m_bGoHome;
}

void CRealXPuck::setBGoHome(bool bGoHome)
{
    m_bGoHome = bGoHome;
}
CRealXPuck& CRealXPuck::GetInstance() {
    static std::auto_ptr<CRealXPuck> pcRealXPuckInstance(new CRealXPuck);
    return *(pcRealXPuckInstance.get());
}

/****************************************/
/****************************************/

CRealXPuck::CRealXPuck() :
    m_bGoHome(false),
...
void CRealXPuck::Init(const std::string& str_config_file_name,
                      const std::string& str_controller_id) {
    RegisterSystemSignalHandlers();
    InitSerial();
    CRealXPuckI2CDevice::Init();
    WakeUpPic();
    SendSensorConfiguration();
...
void CRealXPuck::SendSensorConfiguration() {
    switch (m_eBoardState){
    case WAITING:
    case INITIALIZATION:
    case SENDING_CONFIGURATION:
...
void CRealXPuck::SendActuatorData() {
    switch (m_eBoardState) {
    case SENDING_CONFIGURATION:
    case WAITING:
    case INITIALIZATION:
...
/*
    ...
    */
void CRealXPuck::ReceiveSensorData() {
    switch (m_eBoardState) {
    case WAITING:
    case INITIALIZATION:
    case WAKE_UP:
...
/*
    ...
    *   configuration file.
    */
CCI_Actuator* CRealXPuck::InsertActuator(const std::string& str_actuator_name) {
    if (str_actuator_name == "xpuck_wheels") {
        CRealXPuckWheelsActuator* pcWheelsActuator = CreateActuator<CRealXPuckWheelsActuator>(str_actuator_name);
        pcWheelsActuator->SetState(m_sActuatorState);
        return pcWheelsActuator;
    } else if (str_actuator_name == "xpuck_base_leds") {
        CRealXPuckBaseLEDsActuator* pcLEDsActuator = CreateActuator<CRealXPuckBaseLEDsActuator>(str_actuator_name);
        pcLEDsActuator->SetState(m_sActuatorState);
        return pcLEDsActuator;
    } else if (str_actuator_name == "xpuck_rgb_leds") {
        CRealXPuckRGBLEDsActuator* pcRGBLEDsActuator = CreateActuator<CRealXPuckRGBLEDsActuator>(str_actuator_name);
        m_vecI2CActuators.push_back(pcRGBLEDsActuator);
        return pcRGBLEDsActuator;
    } else if (str_actuator_name == "xpuck_range_and_bearing") {
        CRealXPuckRangeAndBearingActuator* pcRABActuator = CreateActuator<CRealXPuckRangeAndBearingActuator>(str_actuator_name);
        m_vecI2CActuators.push_back(pcRABActuator);
        return pcRABActuator;
    } else if (str_actuator_name == "xpuck_virtual_camrab") {     //@antoine
        CRealXPuckVirtualCamrabActuator* pcVirtualCamrabActuator = CreateActuator<CRealXPuckVirtualCamrabActuator>(str_actuator_name);
        m_vecI2CActuators.push_back(pcVirtualCamrabActuator);
        return pcVirtualCamrabActuator;
    } else if (str_actuator_name == "xpuck_ircom") {
        CRealXPuckIRComActuator* pcIRComActuator = CreateActuator<CRealXPuckIRComActuator>(str_actuator_name);
...
/*
    ...
    *   configuration file.
    */
CCI_Sensor* CRealXPuck::InsertSensor(const std::string& str_sensor_name) {
    if (str_sensor_name == "xpuck_proximity") {
        CRealXPuckProximitySensor* pcProximitySensor = CreateSensor<CRealXPuckProximitySensor>(str_sensor_name);
        pcProximitySensor->SetState(m_sSensorState);
        m_vecSerialSensors.push_back(pcProximitySensor);
        return pcProximitySensor;
    } else if (str_sensor_name == "xpuck_light") {
        CRealXPuckLightSensor* pcLightSensor = CreateSensor<CRealXPuckLightSensor>(str_sensor_name);
        pcLightSensor->SetState(m_sSensorState);
        m_vecSerialSensors.push_back(pcLightSensor);
        return pcLightSensor;
    } else if (str_sensor_name == "xpuck_ground") {
        CRealXPuckGroundSensor* pcGroundSensor = CreateSensor<CRealXPuckGroundSensor>(str_sensor_name);
        m_vecI2CSensors.push_back(pcGroundSensor);
        return pcGroundSensor;
    } else if (str_sensor_name == "xpuck_accelerometer") {
        CRealXPuckAccelerometerSensor* pcAccelerometerSensor = CreateSensor<CRealXPuckAccelerometerSensor>(str_sensor_name);
    ...
    } else if (str_sensor_name == "xpuck_range_and_bearing") {
        CRealXPuckRangeAndBearingSensor* pcRABSensor = CreateSensor<CRealXPuckRangeAndBearingSensor>(str_sensor_name);
        m_vecI2CSensors.push_back(pcRABSensor);
        return pcRABSensor;
    } else if (str_sensor_name == "xpuck_ircom") {
        CRealXPuckIRComSensor* pcIRComSensor = CreateSensor<CRealXPuckIRComSensor>(str_sensor_name);
        pcIRComSensor->SetState(m_sSensorState);
        m_vecSerialSensors.push_back(pcIRComSensor);
        return pcIRComSensor;
    } else if (str_sensor_name == "xpuck_micro") {
        CRealXPuckMicroSensor* pcMicroSensor = CreateSensor<CRealXPuckMicroSensor>(str_sensor_name);
        pcMicroSensor->SetState(m_sSensorState);
        m_vecSerialSensors.push_back(pcMicroSensor);
        return pcMicroSensor;
    } else if (str_sensor_name == "xpuck_omnidirectional_camera") {
        m_pcOmnidirectionalCameraSensor = CreateSensor<CRealXPuckOmnidirectionalCameraSensor>(str_sensor_name);
        return m_pcOmnidirectionalCameraSensor;    
    } else if (str_sensor_name == "xpuck_virtual_camrab") {
        CRealXPuckVirtualCamrabSensor* pcVirtualCamrabSensor = CreateSensor<CRealXPuckVirtualCamrabSensor>(str_sensor_name);
        m_vecI2CSensors.push_back(pcVirtualCamrabSensor);
        return pcVirtualCamrabSensor;
    }if (str_sensor_name == "xpuck_battery") {
        CRealXPuckBatterySensor* pcBatterySensor = CreateSensor<CRealXPuckBatterySensor>(str_sensor_name);
...
bool CRealXPuck::IsExperimentFinished() {
...
/* this function ensure that the control step is synchronized with the
    ...
    */
void CRealXPuck::SyncControlStep() {
...
/*
    ...
    */
void CRealXPuck::InitController(const std::string& str_config_file_name,
                                const std::string& str_controller_id) {
    try {
...
TConfigurationNode CRealXPuck::CreateController(const std::string& str_controller_id,
...
void CRealXPuck::InitController(TConfigurationNode& c_controller_tree) {
...
void CRealXPuck::LoadActuators(TConfigurationNode& c_controller_tree) {
    if (m_pcController != NULL) {
...
void CRealXPuck::LoadSensors(TConfigurationNode& c_controller_tree) {
    if (m_pcController != NULL) {
...
void CRealXPuck::RegisterSystemSignalHandlers() {
    /* termination requested by os (kill) */
...
void CRealXPuck::SignalHandler(SInt32 n_signal) {
...
void CRealXPuck::SetId(CCI_Controller& c_controller) {
    ...
    if (::gethostname(pchBuffer, 30) == 0) {
        ...
        LOGERR << "[WARNING] Failed to get hostname."
               << "Setting default robot id 'x-puck'."
               << std::endl;
        c_controller.SetId("x-puck");
    }
}
