/**
 * @file <argos3/plugins/robots/x-puck/real_robot/real_xpuck_range_and_bearing_sensor.cpp>
 *
 * @author Gianpiero Francesca - <gianpiero.francesca@ulb.ac.be>
 * @author Lorenzo Garattoni - <lgaratto@ulb.ac.be>
 */

#include "real_xpuck_range_and_bearing_sensor.h"
...
CRealXPuckRangeAndBearingSensor::CRealXPuckRangeAndBearingSensor() :
...
void CRealXPuckRangeAndBearingSensor::Init(TConfigurationNode& t_node) {
    CCI_XPuckRangeAndBearingSensor::Init(t_node);
    ...
    try {
        ...
        /* This sleep avoid writing too fast to the bus */
        ::usleep(TIME_BETWEEN_TWO_RECEIVE);
        LOG << "[INFO] Real Xpuck RAB sensor data_size=" << m_uDataSize << std::endl;
        /* Set data size */
        WriteRegister(m_tDeviceStream,
...
void CRealXPuckRangeAndBearingSensor::ReceiveData() {
...
void CRealXPuckRangeAndBearingSensor::Calibrate(const std::string& str_conf_file) {
    try {
...
void CRealXPuckRangeAndBearingSensor::MessageFetcherThread() {
    ...
    while(true) {
        ...
        if(ReadRegister(m_tDeviceStream,
            ...
            CCI_XPuckRangeAndBearingSensor::SReceivedPacket* psPacket =
                    new CCI_XPuckRangeAndBearingSensor::SReceivedPacket();
            if(m_uDataSize > 3) {
...
void* CRealXPuckRangeAndBearingSensor::StartThread(void* pt_arg) {
    reinterpret_cast<CRealXPuckRangeAndBearingSensor*>(pt_arg)->MessageFetcherThread();
    return NULL;
}
