/**
 * @file <argos3/plugins/robots/x-puck/real_robot/real_xpuck_range_and_bearing_actuator.cpp>
 *
 * @author Lorenzo Garattoni - <lgaratto@ulb.ac.be>
 < */

#include "real_xpuck_range_and_bearing_actuator.h"
...
CRealXPuckRangeAndBearingActuator::CRealXPuckRangeAndBearingActuator() {
...
void CRealXPuckRangeAndBearingActuator::Init(TConfigurationNode& t_node) {
    CCI_XPuckRangeAndBearingActuator::Init(t_node);
    ...
    try {
        /*
          ...
          */
        UInt8 unRange = 115;
        /*To understand what e_randb_set_range means, check the xpuck rab's manual*/
        ...
        WriteRegister(m_tDeviceStream,
                      ...
                      unRange);
        LOG << "[INFO] Real Xpuck RAB actuator data_size=" << m_uDataSize << std::endl;
        /* Set data size */
        WriteRegister(m_tDeviceStream,
...
void CRealXPuckRangeAndBearingActuator::SendData() {
...
void CRealXPuckRangeAndBearingActuator::PacketSenderThread() {
    ...
    while(true) {
        ...
        default:
            THROW_ARGOSEXCEPTION("[BUG] CRealXPuckRangeAndBearingActuator::PacketSenderThread() is in an unknown state");
        }
        pthread_mutex_unlock(&m_tDataMutex);
        /*
...
void* CRealXPuckRangeAndBearingActuator::StartThread(void* pt_arg) {
    reinterpret_cast<CRealXPuckRangeAndBearingActuator*>(pt_arg)->PacketSenderThread();
    return NULL;
}
