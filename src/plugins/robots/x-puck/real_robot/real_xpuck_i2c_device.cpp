/**
 * @file <argos3/plugins/robots/x-puck/real_robot/real_xpuck_i2c_device.cpp>
 *
 * @author Lorenzo Garattoni - <lgaratto@ulb.ac.be>
 */

#include "real_xpuck_i2c_device.h"
...
pthread_mutex_t CRealXPuckI2CDevice::m_tIOMutex;
UInt8 maxRetries = 10;

/****************************************/
/****************************************/

void CRealXPuckI2CDevice::Init() {
    ::pthread_mutex_init(&m_tIOMutex, NULL);
}

/****************************************/
/****************************************/
void CRealXPuckI2CDevice::ThreadCleanup(){
...
TI2CDeviceStream CRealXPuckI2CDevice::OpenDevice(SInt16 n_address) {
...
void CRealXPuckI2CDevice::CloseDevice(TI2CDeviceStream t_stream) {
...
UInt16 CRealXPuckI2CDevice::ReadRegister(TI2CDeviceStream t_stream,
                                         SInt8 n_register,
                                         SInt16 n_count) {
    ...
    if(n_count < 1 || n_count > 2) {
        ...
        LOGERR << "CRealXPuckI2CDevice::ReadRegister()"
    ...
    if(nRetries == maxRetries) {
        ...
        LOG << "CRealXPuckI2CDevice::ReadRegister(): "
...
void CRealXPuckI2CDevice::WriteRegister(TI2CDeviceStream t_stream,
                                        SInt8 n_register,
                                        SInt8 n_value) {
...
void CRealXPuckI2CDevice::WriteData(TI2CDeviceStream t_stream,
    ...
    do {
        ...
        if (nWritten < 0 && unRetries > maxRetries) {
            ...
            LOGERR << "CRealXPuckI2CDevice::WriteData(): "
                   << "Unable to send data to the I2C device stream: "
                   << ::strerror(errno);
        } else if (nWritten < 0){
            ++unRetries;
