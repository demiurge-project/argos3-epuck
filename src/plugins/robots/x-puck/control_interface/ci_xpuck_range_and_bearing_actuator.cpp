/**
 * @file <argos3/plugins/robots/x-puck/simulator/xpuck_range_and_bearing_default_actuator.cpp>
 *
 * @author Gianpiero Francesca <gianpiero.francesca@ulb.ac.be>
 * @author Lorenzo Garattoni <lgaratto@ulb.ac.be>
 */

#include "ci_xpuck_range_and_bearing_actuator.h"
...
namespace argos {
   ...
   int LuaRABSetData(lua_State* pt_lua_state) {
      if(lua_gettop(pt_lua_state) == 1) {
         ...
         CCI_XPuckRangeAndBearingActuator* pcAct = CLuaUtility::GetDeviceInstance<CCI_XPuckRangeAndBearingActuator>(pt_lua_state, "range_and_bearing");
         /* Check whether sizes match */
         if(CCI_XPuckRangeAndBearingActuator::MAX_BYTES_SENT != lua_rawlen(pt_lua_state, -1)) {
            return luaL_error(pt_lua_state, "robot.range_and_bearing.set_data(array) expects an array of %d numbers", CCI_XPuckRangeAndBearingActuator::MAX_BYTES_SENT);
         }
         /* Fill up a byte array, checking that all elements are numbers */
         CCI_XPuckRangeAndBearingActuator::TData tBuf;
         for(size_t i = 0; i < CCI_XPuckRangeAndBearingActuator::MAX_BYTES_SENT ; ++i) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_XPuckRangeAndBearingActuator::CreateLuaState(lua_State* pt_lua_state) {
   ...
   void CCI_XPuckRangeAndBearingActuator::Init(TConfigurationNode& t_tree) {
      try {
   ...
   void CCI_XPuckRangeAndBearingActuator::SetData(const TData t_data) {
   ...
   void CCI_XPuckRangeAndBearingActuator::SetDataForEmitter(size_t un_idx,
                                                             const TData t_data) {
   ...
   void CCI_XPuckRangeAndBearingActuator::Disable() {
   ...
   void CCI_XPuckRangeAndBearingActuator::DisableEmitter(size_t un_idx) {
      m_sDesiredData.State = STATE_EMITTERS_DIFFERENT;
   ...
   /****************************************/
   /****************************************/
   CCI_XPuckRangeAndBearingActuator::SDataToSend::SDataToSend() :
   ...
   CCI_XPuckRangeAndBearingActuator::SDataToSend::SDataToSend(const CCI_XPuckRangeAndBearingActuator::SDataToSend& s_data) :
      State(s_data.State) {
      for(size_t i = 0; i < 12; ++i) {
   ...
   CCI_XPuckRangeAndBearingActuator::SDataToSend&
   CCI_XPuckRangeAndBearingActuator::SDataToSend::operator=(
           const CCI_XPuckRangeAndBearingActuator::SDataToSend& s_data) {
      if(this != &s_data) {
         State = s_data.State;
         for(size_t i = 0; i < 12; ++i) {
            Emitter[i].Enabled = s_data.Emitter[i].Enabled;
