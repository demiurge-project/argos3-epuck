/**
 * @file <argos3/plugins/robots/x-puck/control_interface/ci_xpuck_virtual_camrab_sensor.cpp>
 *
 * @author Antoine Ligot - <aligot@ulb.ac.be>
 */

#include "ci_xpuck_virtual_camrab_sensor.h"
...
namespace argos {
    ...
    const Real CCI_XPuckVirtualCamrabSensor::VARIANCE_CAMERA = 1.6;
    const Real CCI_XPuckVirtualCamrabSensor::VARIANCE_RAB = 324.7;

	const Real CCI_XPuckVirtualCamrabSensor::ROBOT_DIAMETER = 7.0;
	const Real CCI_XPuckVirtualCamrabSensor::VIRTUAL_ROBOT_DIAMETER = 10.0;

    const UInt32 CCI_XPuckVirtualCamrabSensor::DEFAULT_NUMBER_COLORS = 3;
	...
	#ifdef ARGOS_WITH_LUA
	   void CCI_XPuckVirtualCamrabSensor::CreateLuaState(lua_State* pt_lua_state) {
	...
	#ifdef ARGOS_WITH_LUA
	   void CCI_XPuckVirtualCamrabSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
    ...
    CCI_XPuckVirtualCamrabSensor::CCI_XPuckVirtualCamrabSensor() {
        m_unNumberColors = CCI_XPuckVirtualCamrabSensor::DEFAULT_NUMBER_COLORS;
    }

	/****************************************/
	/****************************************/

    void CCI_XPuckVirtualCamrabSensor::Init(TConfigurationNode& t_node) {
    ...
    void CCI_XPuckVirtualCamrabSensor::ClearReadings() {
    ...
    bool SortByAngle(CCI_XPuckOmnidirectionalCameraSensor::SBlob* l, CCI_XPuckOmnidirectionalCameraSensor::SBlob* r) {
    ...
    std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> CCI_XPuckVirtualCamrabSensor::FilterBlobs(std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vec_blobs) {
        // Flag used
        ...
        std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vecSameColorBlobs;

        // Will contain blobs emitted by the same robot
        std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vecRobotBlobs;

        CCI_XPuckOmnidirectionalCameraSensor::SBlob* sReferenceBlob;
        CCI_XPuckOmnidirectionalCameraSensor::SBlob* sCurrentBlob;
		...
		for (UInt8 color = 0; color < m_unNumberColors; color++) {
			...
			for (std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*>::iterator it = vecSameColorBlobs.begin(); it != vecSameColorBlobs.end(); ++it) {
    ...
    CColor CCI_XPuckVirtualCamrabSensor::IntegerToColor(UInt8 un_color_id) {
        CColor cColor;
        if (un_color_id == 0) {
            cColor = CColor::RED;
    ...
    UInt8 CCI_XPuckVirtualCamrabSensor::ColorToInteger(CColor c_color) {
        UInt8 unColorId = 0;
    ...
    bool CCI_XPuckVirtualCamrabSensor::SameColor(CColor c_color, UInt8 un_color_id) {
    ...
    std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> CCI_XPuckVirtualCamrabSensor::GetBlobsWithColor(std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vec_blobs, UInt8 un_color) {
        std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vecBlobsWithSameColor;
        for (std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*>::iterator itBlob = vec_blobs.begin(); itBlob != vec_blobs.end(); ++itBlob) {
    ...
    Real CCI_XPuckVirtualCamrabSensor::RadiansToDegrees(CRadians c_angle) {
    ...
    Real CCI_XPuckVirtualCamrabSensor::AverageAngle(std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vec_blobs) {
        Real fResult = 0.0;
        Real fCurrentAngle;
        ...
        for (std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*>::iterator it = vec_blobs.begin(); it != vec_blobs.end(); ++it) {
    ...
    bool CCI_XPuckVirtualCamrabSensor::RelativelyCloseAngle(Real f_angle_1, Real f_angle_2, Real f_criterion) {
    ...
    bool CCI_XPuckVirtualCamrabSensor::RelativelyCloseDistance(Real f_dist_1, Real f_dist_2) {
    ...
    bool CCI_XPuckVirtualCamrabSensor::BelongingToSameRobot(CCI_XPuckOmnidirectionalCameraSensor::SBlob* s_reference_blob, CCI_XPuckOmnidirectionalCameraSensor::SBlob* s_current_blob) {
    ...
    CCI_XPuckOmnidirectionalCameraSensor::SBlob* CCI_XPuckVirtualCamrabSensor::MergeBlobs(std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*> vec_robot_blobs) {
        CColor cRobotColor;
        Real fRobotAngle = 0.0f;
        Real fRobotDistance = 10000.0f;
        UInt32 unRobotArea = 0;
        for (std::vector<CCI_XPuckOmnidirectionalCameraSensor::SBlob*>::iterator it = vec_robot_blobs.begin(); it != vec_robot_blobs.end(); ++it) {
        ...
        return new CCI_XPuckOmnidirectionalCameraSensor::SBlob(cRobotColor, cRobotAngleRad, fRobotDistance, unRobotArea);
    }

    /****************************************/
    /****************************************/

    Real CCI_XPuckVirtualCamrabSensor::Fusion(Real f_blob_distance, Real f_RAB_distance) {
