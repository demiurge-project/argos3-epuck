/**
 * @file <argos3/plugins/robots/x-puck/control_interface/ci_xpuck_RGB_leds_actuator.h
 *
 * @author Lorenzo Garattoni - <lgaratto@ulb.ac.be>
 */

#include "ci_xpuck_rgb_leds_actuator.h"
...
namespace argos {
   ...
   int LuaLEDSetColor(lua_State* pt_lua_state) {
      ...
      CCI_XPuckRGBLEDsActuator* pcAct = CLuaUtility::GetDeviceInstance<CCI_XPuckRGBLEDsActuator>(pt_lua_state, "overo_leds");
      if(unIdx < 1 || unIdx > 3) {
   ...
   int LuaLEDSetColors(lua_State* pt_lua_state) {
      ...
      /* Perform action */
      CLuaUtility::GetDeviceInstance<CCI_XPuckRGBLEDsActuator>(pt_lua_state, "overo_leds")->SetColors(cColor);
      return 0;
   }
#endif
   ...
   CCI_XPuckRGBLEDsActuator::CCI_XPuckRGBLEDsActuator() :
      m_tLEDSettings(3) {}

   /****************************************/
   /****************************************/

   void CCI_XPuckRGBLEDsActuator::SetColor(size_t un_led_number,
   ...
   void CCI_XPuckRGBLEDsActuator::SetColors(const CColor& c_color) {
      SetColor(0, c_color);
      SetColor(1, c_color);
      SetColor(2, c_color);
   }
   ...
   void CCI_XPuckRGBLEDsActuator::SetColors(const TLEDSettings& t_colors) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_XPuckRGBLEDsActuator::CreateLuaState(lua_State* pt_lua_state) {
