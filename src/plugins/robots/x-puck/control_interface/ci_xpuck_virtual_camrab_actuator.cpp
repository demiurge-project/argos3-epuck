/**
 * @file <argos3/plugins/robots/x-puck/control_interface/ci_xpuck_virtual_camrab_actuator.cpp>
 *
 * @author Antoine Ligot - <aligot@ulb.ac.be>
 */

#include "ci_xpuck_virtual_camrab_actuator.h"
...
namespace argos {
    ...
    const UInt32 CCI_XPuckVirtualCamrabActuator::DEFAULT_NUMBER_COLORS = 3;
    ...
    */
    int LuaCamrabSetData(lua_State* pt_lua_state) {
        if(lua_gettop(pt_lua_state) == 1) {
            ...
            CCI_XPuckVirtualCamrabActuator* pcAct = CLuaUtility::GetDeviceInstance<CCI_XPuckVirtualCamrabActuator>(pt_lua_state, "virtual_camrab");
            /* Check whether sizes match */
            if(CCI_XPuckVirtualCamrabActuator::MAX_BYTES_SENT != lua_rawlen(pt_lua_state, -1)) {
                return luaL_error(pt_lua_state, "robot.virtual_camrab.set_data(array) expects an array of %d numbers", CCI_XPuckVirtualCamrabActuator::MAX_BYTES_SENT);
            }
            /* Fill up a byte array, checking that all elements are numbers */
            CCI_XPuckVirtualCamrabActuator::TData tBuf;
            for(size_t i = 0; i < CCI_XPuckVirtualCamrabActuator::MAX_BYTES_SENT ; ++i) {
    ...
    #ifdef ARGOS_WITH_LUA
    void CCI_XPuckVirtualCamrabActuator::CreateLuaState(lua_State* pt_lua_state) {
    ...
    CCI_XPuckVirtualCamrabActuator::CCI_XPuckVirtualCamrabActuator() {
        m_bLEDFlag = false;
        m_pcRandomNumberGenerator = CRandom::CreateRNG("argos");
        m_unNumberColors = CCI_XPuckVirtualCamrabActuator::DEFAULT_NUMBER_COLORS;
    }

    /****************************************/
    /****************************************/

    void CCI_XPuckVirtualCamrabActuator::Init(TConfigurationNode& t_tree) {
        try {
    ...
    void CCI_XPuckVirtualCamrabActuator::SetData(const TData t_data) {
    ...
    void CCI_XPuckVirtualCamrabActuator::SetDataForEmitter(size_t un_idx, const TData t_data) {
    ...
    CCI_XPuckVirtualCamrabActuator::SDataToSend::SDataToSend() :
    ...
    CCI_XPuckVirtualCamrabActuator::SDataToSend::SDataToSend(const CCI_XPuckVirtualCamrabActuator::SDataToSend& s_data) :
        State(s_data.State) {
        for(size_t i = 0; i < 12; ++i) {
    ...
    CCI_XPuckVirtualCamrabActuator::SDataToSend&
    CCI_XPuckVirtualCamrabActuator::SDataToSend::operator=(
           const CCI_XPuckVirtualCamrabActuator::SDataToSend& s_data) {
        if(this != &s_data) {
            State = s_data.State;
            for(size_t i = 0; i < 12; ++i) {
                Emitter[i].Enabled = s_data.Emitter[i].Enabled;
    ...
    void CCI_XPuckVirtualCamrabActuator::Disable() {
    ...
    void CCI_XPuckVirtualCamrabActuator::PickRandomColor() {
    ...
    CColor CCI_XPuckVirtualCamrabActuator::IntegerToColor(UInt8 color_id) {
        CColor cColor;
        if (color_id == 0) {
            cColor = CColor::RED;
        } else if (color_id == 1) {
