/**
 * @file <argos3/plugins/robots/x-puck/control_interface/ci_xpuck_proximity_sensor.h>
 *
 * @brief This file provides the definition of the xpuck proximity
 * sensor.
 *
 * This file provides the definition of the xpuck proximity sensor.
 * The sensors are spaced on a ring around the base of the xpuck.
 * Therefore, they do not turn with the turret. The readings are in
 * the following order (seeing the xpuck from TOP, battery socket is
 * the BACK):
 *
 *              front
 *
...
#ifndef CCI_XPUCK_PROXIMITY_SENSOR_H
#define CCI_XPUCK_PROXIMITY_SENSOR_H

namespace argos {
   class CCI_XPuckProximitySensor;
}
...
namespace argos {
   class CCI_XPuckProximitySensor : public CCI_Sensor {
   ...
   public:
       ...
       * Constructor
       */
      CCI_XPuckProximitySensor();

      /**
       * Destructor
       */
      virtual ~CCI_XPuckProximitySensor() {}
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_XPuckProximitySensor::SReading& s_reading);

   /**
    * Redefine the "<<" operator for multiple readings
    */
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_XPuckProximitySensor::TReadings& t_readings);
}
#endif
