/**
 * @file <argos3/plugins/robots/x-puck/control_interface/ci_xpuck_base_leds_actuator.cpp
 *
 * @author Lorenzo Garattoni- <lgaratto@ulb.ac.be>
 */

#include "ci_xpuck_base_leds_actuator.h"
...
namespace argos {
   ...
   int LuaSwitchLED(lua_State* pt_lua_state) {
      ...
      /* Perform action */
      CLuaUtility::GetDeviceInstance<CCI_XPuckBaseLEDsActuator>(pt_lua_state, "base_leds")->
         SwitchLED(unIdx - 1,
   ...
   CCI_XPuckBaseLEDsActuator::CCI_XPuckBaseLEDsActuator() :
      m_unLEDSettings(0) {
   }
   ...
   void CCI_XPuckBaseLEDsActuator::SwitchLED(size_t un_led_number,
                                             bool b_switched_on) {
      if(b_switched_on) {
   ...
   void CCI_XPuckBaseLEDsActuator::SwitchLEDs(bool b_switched_on) {
      if(b_switched_on) {
         m_unLEDSettings = 255;
      } else {
         m_unLEDSettings = 0;
...
#ifdef ARGOS_WITH_LUA
   void CCI_XPuckBaseLEDsActuator::CreateLuaState(lua_State* pt_lua_state) {
