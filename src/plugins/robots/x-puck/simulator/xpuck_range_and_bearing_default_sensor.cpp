/**
 * @file <argos3/plugins/robots/x-puck/simulator/xpuck_rab_default_sensor.cpp>
 *
 *
 ...
 */
#include <limits>
#include "xpuck_range_and_bearing_default_sensor.h"
...
namespace argos {
static const Real XPUCK_RADIUS = 3.5f;
/****************************************/
/****************************************/
...
CXPuckRABDefaultSensor::CXPuckRABDefaultSensor() :
...
void CXPuckRABDefaultSensor::SetRobot(CComposableEntity& c_entity) {
...
void CXPuckRABDefaultSensor::Init(TConfigurationNode& t_tree) {
    try {
        /* Parent class init */
        CCI_XPuckRangeAndBearingSensor::Init(t_tree);
        /* Show rays? */
...
void CXPuckRABDefaultSensor::Update() {
    ...
    CCI_XPuckRangeAndBearingSensor::SReceivedPacket* psReceivedPacket = NULL;
    /* Pointer to the RAB entity sending the packet */
    CXpuckRABEquippedEntity* pcRABSendingEquipedEntity = NULL;
    /* Go through communicating RABs and create packets */
    size_t unSize=setRABs.size();
    CXpuckRABEquippedEntity* vRABentities[unSize];
	...
	vRABentities[indexes.at(selectedIndex)]=static_cast<CXpuckRABEquippedEntity*>(*itRABSendingEquipedEntity);
    ...
    for (size_t i = 0; i < unSize && unMessagesNeeded>0; ++i) {
         ...
         */
        CCI_XPuckRangeAndBearingActuator::SEmitter& em=pcRABSendingEquipedEntity->GetEmitterForAngle(cReceiverToSenderAngle);
        if(em.Enabled){
            ...
            if(m_fLossProbability==0.0f || !m_pcRNG->Bernoulli(m_fLossProbability)){
                ...
                psReceivedPacket = new CCI_XPuckRangeAndBearingSensor::SReceivedPacket();
                ...
                psReceivedPacket->Range-=2*XPUCK_RADIUS;
                if(m_bRealRangeNoise){
                    /** Add noise
...
void CXPuckRABDefaultSensor::Reset() {
    ClearPackets();
}
...
void CXPuckRABDefaultSensor::Destroy() {
    ClearPackets();
...
REGISTER_SENSOR(CXPuckRABDefaultSensor,
                "xpuck_range_and_bearing", "medium",
                "Gianpiero Francesca - [gianpiero.francesca@ulb.ac.be]"
                "Lorenzo Garattoni- [lgaratto@ulb.ac.be]",
                "2.0",
                "The range-and-bearing sensor of the x-puck.",
                ...
                "range-and-bearing actuator (\"xpuck_range_and_bearing\" in the actuator section).\n"
                "For use, refer to [ci_xpuck_range_and_bearing_sensor.h].\n\n"
                ...
                "         <sensors>\n"
                "            ...\n"
                "            <xpuck_range_and_bearing implementation=\"medium\" medium=\"rab\" show_rays=\"true\"/>\n"
                "            ...\n"
                "         <sensors/>\n"
                "         ...\n"
                "      <my_controller/>\n"
