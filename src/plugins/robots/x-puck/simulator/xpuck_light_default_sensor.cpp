/**
 * @file <argos3/plugins/robots/x-puck/simulator/xpuck_light_default_sensor.cpp>
 *
 * @author Lorenzo Garattoni - <lgaratto@ulb.ac.be>
 */

#include "xpuck_light_default_sensor.h"
...
namespace argos {
   ...
   CXPuckLightDefaultSensor::CXPuckLightDefaultSensor() :
   ...
   void CXPuckLightDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the x-puck light default sensor", ex);
      }
   }
   ...
   void CXPuckLightDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in x-puck default light sensor", ex);
      }
   }
   ...
   void CXPuckLightDefaultSensor::Update() {
      /* Erase readings */
      for(size_t i = 0; i < m_tReadings.size(); ++i) {
         m_tReadings[i].Value = 0.0f;
      }
      /* Get x-puck orientation */
      ...
      /* Buffer for the angle of the light wrt to the x-puck */
      CRadians cAngleLightWrtXPuck;
      ...
      for(CSpace::TMapPerType::iterator it = mapLights.begin();
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            /* Check occlusion between the x-puck and the light */
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               /* Get the distance between the light and the x-puck */
               ...
               /* Get the angle wrt to x-puck rotation */
               cAngleLightWrtXPuck = cRobotToLight.GetZAngle();
               cAngleLightWrtXPuck -= cOrientationZ;
               /* Set the actual readings */
               Real fReading = cRobotToLight.Length();
               /*
                * Take 8 readings and delete the ones with angle > pi/2 - thus 
                * we process sensors that are within 180 degrees of intersection
                * of light ray with robot body
                */
               for(SInt32 nIndexOffset = 0; nIndexOffset < 8; ++nIndexOffset) {
                  cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtXPuck - m_tReadings[nIndexOffset].Angle).SignedNormalize());
                  ...
                  if (cAngularDistanceFromOptimalLightReceptionPoint < CRadians::PI_OVER_TWO)
                     ...
                     * 0) to 0 (dist PI/2)
                     */
                    m_tReadings[nIndexOffset].Value += ComputeReading(fReading) * ScaleReading(cAngularDistanceFromOptimalLightReceptionPoint);
               }
            }
            else {
               /* The ray is occluded */
   ...
   void CXPuckLightDefaultSensor::Reset() {
   ...
   REGISTER_SENSOR(CXPuckLightDefaultSensor,
                   "xpuck_light", "default",
                   "Lorenzo Garattoni [lgaratto@ulb.ac.be]",
                   "1.0",
                   "The x-puck light sensor",
                   ...
                   "For usage, refer to [ci_xpuck_light_sensor.h]\n\n"
                   ...
                   "         <sensors>\n"
                   "            ...\n"
                   "            <xpuck_light implementation=\"default\"/>\n"
                   "            ...\n"
                   "         <sensors/>\n"
                   "         ...\n"
                   "      <my_controller/>\n"
                   ...
                   "         <sensors>\n"
                   "            ...\n"
                   "            <xpuck_light implementation=\"default\" show_rays=\"true\"/>\n"
                   "            ...\n"
                   "         <sensors/>\n"
                   "         ...\n"
                   "      <my_controller/>\n"
                   ...
                   "         <sensors>\n"
                   "            ...\n"
                   "            <xpuck_light implementation=\"default\" noise_level=\"0.1\"/>\n"
                   "            ...\n"
                   "         <sensors/>\n"
                   "         ...\n"
                   "      <my_controller/>\n"
